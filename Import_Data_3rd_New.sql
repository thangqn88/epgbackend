CREATE DEFINER=`kplus`@`%` PROCEDURE `Import_Data_3rd_New`()
BEGIN
...
DECLARE msg TEXT DEFAULT '0';
DECLARE v_check_running TINYINT(1);
DECLARE v_count INT;
DECLARE v_step VARCHAR(100) DEFAULT '0';

DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
      GET DIAGNOSTICS CONDITION 1
        code = RETURNED_SQLSTATE, msg = MESSAGE_TEXT;
        CALL Processing_InsertLog(code,'Import_Data_3rd_New',CONCAT(v_step,': ',msg));
        CALL epgbackend.Proc_UpSert_Running_Status('Import_Data_3rd_New',0,-1);
        SELECT msg;
    END;
...
SET v_check_running = epgbackend.Func_Check_Running_Status('Import_Data_3rd_New');
IF v_check_running = 0 THEN
	CALL epgbackend.Proc_UpSert_Running_Status('Import_Data_3rd_New',1,0);
	...
	INSERT INTO tmp_epg_record
		...
		where t.status = '0'
		AND t.from_source != 'HDVN'
		and t.channel_name not in ('VTV1','VTV2','VTV3','VTV4','VTV5','VTV6','VTV7','VTV8','VTV9')
		AND CASE WHEN  t.from_source = 'HDVN' THEN
			epgbackend.Process_CalculateDuration(t.start_date,t.start_time,t.end_date,t.end_time) >= 180
		ELSE 1=1 END
	...
	ELSE
        ...
        SET v_step = 'Insert to table broadcast if not exists';
        ...
        SET v_step = 'Update table broadcast if exists';
		SET SQL_SAFE_UPDATES = 0;
        ...
        SET v_step = 'Insert into table program if not exists';
        ...
        SET v_step = 'Update table program if exists';
		SET SQL_SAFE_UPDATES = 0;
		...
		# -- ReUpdate Program ID
        SET v_step = 'ReUpdate Program ID';
		SET SQL_SAFE_UPDATES = 0;
        ...
        SET v_step = 'Upsert data to export';
		SET SQL_SAFE_UPDATES = 0;
	...
	END IF;
		CALL epgbackend.Proc_UpSert_Running_Status('Import_Data_3rd_New',0,v_count);
END IF;
END