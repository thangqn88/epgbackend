CREATE DEFINER=`kplus`@`%` PROCEDURE `Import_Data_Kplus_New`()
BEGIN
...
DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
      GET DIAGNOSTICS CONDITION 1
        ...
        CALL Processing_InsertLog(code,'Import_Data_Kplus_New',msg);
        CALL epgbackend.Proc_UpSert_Running_Status('Import_Data_Kplus_New',0, 0);
    END;
    
SET v_check_running = epgbackend.Func_Check_Running_Status('Import_Data_Kplus_New');
IF v_check_running = 0 THEN
	CALL epgbackend.Proc_UpSert_Running_Status('Import_Data_Kplus_New',1, 0);
	...
	TRUNCATE TABLE tmp_epg_record;

	INSERT INTO tmp_epg_record
		SELECT t.* FROM epg_record t
		WHERE t.status = '0'
		AND t.from_source = 'HDVN'
		AND epgbackend.Process_CalculateDuration(t.start_date,t.start_time,t.end_date,t.end_time) >= 180
	...
	SET SQL_SAFE_UPDATES = 1;
	END IF;
    CALL epgbackend.Proc_UpSert_Running_Status('Import_Data_Kplus_New',0,v_count);
ELSE
	SELECT 'Another process is running';
END IF;
